...
unsigned int CYSFConvolution::chainback(uint8_t* out, unsigned int nBits)
{
	assert(out != NULL);
...
void CYSFConvolution::encode(const uint8_t* in, uint8_t* out, unsigned int nBits) const
{
	assert(in != NULL);
	assert(out != NULL);
	assert(nBits > 0U);
