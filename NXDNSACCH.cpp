...
bool CNXDNSACCH::decode(const uint8_t* data)
{
	assert(data != nullptr);
...
void CNXDNSACCH::encode(uint8_t* data) const
{
	assert(data != nullptr);
...
uint8_t CNXDNSACCH::getRAN() const
{
	return m_data[0U] & 0x3FU;
}

uint8_t CNXDNSACCH::getStructure() const
{
	return (m_data[0U] >> 6) & 0x03U;
}

void CNXDNSACCH::getData(uint8_t* data) const
{
	assert(data != nullptr);
...
void CNXDNSACCH::getRaw(uint8_t* data) const
{
	assert(data != nullptr);
...
void CNXDNSACCH::setRAN(uint8_t ran)
{
	m_data[0U] &= 0xC0U;
	m_data[0U] |= ran;
}

void CNXDNSACCH::setStructure(uint8_t structure)
{
	m_data[0U] &= 0x3FU;
	m_data[0U] |= (structure << 6) & 0xC0U;
}

void CNXDNSACCH::setData(const uint8_t* data)
{
	assert(data != nullptr);
...
void CNXDNSACCH::setRaw(const uint8_t* data)
{
	assert(data != nullptr);
