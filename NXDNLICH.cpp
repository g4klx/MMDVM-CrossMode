...
bool CNXDNLICH::decode(const uint8_t* bytes)
{
...
void CNXDNLICH::encode(uint8_t* bytes)
{
	assert(bytes != nullptr);
...
uint8_t CNXDNLICH::getRFCT() const
{
	return (m_lich[0U] >> 6) & 0x03U;
}

uint8_t CNXDNLICH::getFCT() const
{
	return (m_lich[0U] >> 4) & 0x03U;
}

uint8_t CNXDNLICH::getOption() const
{
	return (m_lich[0U] >> 2) & 0x03U;
}

uint8_t CNXDNLICH::getDirection() const
{
	return (m_lich[0U] >> 1) & 0x01U;
}

uint8_t CNXDNLICH::getRaw() const
{
	bool parity = getParity();
	if (parity)
		m_lich[0U] |= 0x01U;
...
void CNXDNLICH::setRFCT(uint8_t rfct)
{
	m_lich[0U] &= 0x3FU;
	m_lich[0U] |= (rfct << 6) & 0xC0U;
}

void CNXDNLICH::setFCT(uint8_t usc)
{
	m_lich[0U] &= 0xCFU;
	m_lich[0U] |= (usc << 4) & 0x30U;
}

void CNXDNLICH::setOption(uint8_t option)
{
	m_lich[0U] &= 0xF3U;
	m_lich[0U] |= (option << 2) & 0x0CU;
}

void CNXDNLICH::setDirection(uint8_t direction)
{
	m_lich[0U] &= 0xFDU;
	m_lich[0U] |= (direction << 1) & 0x02U;
}

void CNXDNLICH::setRaw(uint8_t lich)
{
	m_lich[0U] = lich;
}
