...
bool CDMRNetwork::writeRadioPosition(unsigned int id, const uint8_t* data)
{
	uint8_t buffer[20U];
...
bool CDMRNetwork::writeTalkerAlias(unsigned int id, uint8_t type, const uint8_t* data)
{
	uint8_t buffer[20U];
...
bool CDMRNetwork::write(const uint8_t* data, unsigned int length)
{
	assert(data != NULL);
	assert(length > 0U);
