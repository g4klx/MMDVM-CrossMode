/*
 *   Copyright (C) 2009-2016,2018,2021,2024 by Jonathan Naylor G4KLX
 *
...
unsigned int CNXDNConvolution::chainback(uint8_t* out, unsigned int nBits)
{
	assert(out != nullptr);
...
void CNXDNConvolution::encode(const uint8_t* in, uint8_t* out, unsigned int nBits) const
{
	assert(in != nullptr);
	assert(out != nullptr);
	assert(nBits > 0U);
